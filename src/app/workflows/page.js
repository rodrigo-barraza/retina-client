...
export default function WorkflowsPage({ initialWorkflowId }) {
  ...
  const handleChangeModel = useCallback((nodeId, newModel) => {
    ...
    setEdges((prev) =>
      prev.filter((c) => {
        if (c.targetNodeId === nodeId && !newInputTypes.has(c.targetModality))
          return false;
        if (c.sourceNodeId === nodeId && !newOutputTypes.has(c.sourceModality))
          return false;
        return true;
      }),
    );
