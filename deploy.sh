...
#
# Builds the Docker image locally (Next.js standalone), pipes
# it over SSH to the NAS, and restarts the container — zero
# manual UI steps.
#
# Usage:
...
NAS_COMPOSE_DIR="/volume1/docker/retina"  # Synology path where compose lives
NAS_SMB_DIR="/mnt/k/retina"              # Fallback: Windows SMB mount
...
echo -e "${CYAN}${BOLD}  👁️ Retina — Deploy to Synology${RESET}"
if $DRY_RUN; then
...
# Source deploy env vars — Vault credentials are needed at build time
# for next.config.mjs to resolve secrets
BUILD_ARGS=""
...
if $HAS_SSH; then
  ...
  if $DRY_RUN; then
    ...
    info "  2. Project → retina → Stop → Start"
  fi
fi
